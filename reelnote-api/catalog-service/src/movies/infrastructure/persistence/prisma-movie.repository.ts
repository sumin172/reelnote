...
import {
  CatalogPrismaAccessor,
  CatalogPrismaTransaction,
} from "../../../infrastructure/db/catalog-prisma.accessor.js";
import {
  MovieRepositoryPort,
  SaveManyOptions,
  SaveOptions,
...
export class PrismaMovieRepository extends MovieRepositoryPort {
    ...
    for (const chunk of this.chunkMovies(movies, chunkSize)) {
      const savedChunk = await this.prisma.runInTransaction(async (tx) => {
        ...
        for (const movie of chunk) {
          const saved = await this.saveWithinTransaction(
            tx,
            movie.toSnapshot(),
            strategy,
          );
  ...
  private async syncGenresFullReplace(
    tx: Tx,
    tmdbId: number,
    genres: string[],
  ): Promise<void> {
  ...
  private async syncGenresWithDiff(
    tx: Tx,
    tmdbId: number,
    ...
    const currentByName = new Map(
      currentRelations.map((rel) => [rel.genre.name, rel.genreId]),
    );
  ...
  private async syncKeywordsFullReplace(
    tx: Tx,
    tmdbId: number,
    keywords: string[],
  ): Promise<void> {
  ...
  private async syncKeywordsWithDiff(
    tx: Tx,
    tmdbId: number,
    ...
    const currentByName = new Map(
      currentRelations.map((rel) => [rel.keyword.name, rel.keywordId]),
    );
    ...
    const toAddNames = desiredKeywords.filter(
    ...
    if (toAddNames.length > 0) {
      ...
      const data = toAddNames
        .map((name) => keywordMap.get(name))
        .filter(
          (keywordId): keywordId is number => typeof keywordId === "number",
        )
        .map((keywordId) => ({ tmdbId, keywordId }));
  ...
  private async saveWithinTransaction(
    tx: Tx,
  ...
  private shouldSkipUpdate(
    ...
    const existingSourceUpdatedAt =
      existingMeta.sourceUpdatedAt?.getTime() ?? null;

    return (
      existingMeta.sourceHash === snapshot.sourceHash &&
      snapshotSourceUpdatedAt === existingSourceUpdatedAt
    );
  }
