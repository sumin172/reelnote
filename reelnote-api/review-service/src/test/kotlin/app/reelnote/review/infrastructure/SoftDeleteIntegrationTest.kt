...
class SoftDeleteIntegrationTest : TestcontainersBase() {
    ...
    fun `소프트 삭제된 리뷰는 기본 조회에서 제외됨`() {
        ...
        val nativeQuery =
            entityManager.entityManager.createNativeQuery(
                "SELECT deleted, deleted_at FROM ${SoftDeleteConfig.SOFT_DELETE_SCHEMA}.reviews WHERE id = ?",
            )
    ...
    fun `낙관적 락과 함께 소프트 삭제가 정상 작동함`() {
        ...
        val nativeQuery =
            entityManager.entityManager.createNativeQuery(
                "SELECT version, deleted FROM ${SoftDeleteConfig.SOFT_DELETE_SCHEMA}.reviews WHERE id = ?",
            )
        nativeQuery.setParameter(1, savedReview.id)
        val result = nativeQuery.singleResult as Array<*>
